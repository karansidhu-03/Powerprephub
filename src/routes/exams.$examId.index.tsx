...
import { ArrowRight, BookMarked, FileText, Lock } from "lucide-react";

import { getExam, type Paper } from "@/data/exams";
        ...
        {exam.chapters.length > 0 && (
          <section>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Focused practice sets organized by individual chapter topics.
            </p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {exam.chapters.map((paper) => (
                <PaperCard key={paper.id} paper={paper} examId={exam.id} />
              ))}
            </div>
          </section>
        )}

        <section className={exam.chapters.length > 0 ? "mt-14" : ""}>
          <div className="flex items-center gap-2.5">
            <FileText className="size-5 text-accent" />
            <h2 className="text-xl font-bold sm:text-2xl">
              Full 100 Question Papers
            </h2>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            Complete full-length practice exams simulating the real test
            experience.
          </p>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
...
function PaperCard({ paper, examId }: { paper: Paper; examId: string }) {
  ...
  return ready ? (
    <Link
      ...
      params={{ examId, paperId: paper.id }}
    >
      {card}
    </Link>
  ) : (
